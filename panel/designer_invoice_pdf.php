...
$user = current_user();
// Access: admins / finance (view_invoices), or a designer with view_own_invoices.
...
// A designer may only open PDF of their OWN invoice.
if (!is_admin() && !has_permission('view_invoices') && (int) $invoice['designer_id'] !== (int) $user['id']) {
    http_response_code(403);
    die('دسترسی غیرمجاز');
}
$items = getDesignerInvoiceItems($invoiceId);

$designerName = $invoice['designer_name'] ?? 'طراح';
$periodLabel = $invoice['period_label'] ?? '';
$invoiceTitle = 'صورت حساب طراحی ' . $designerName . ($periodLabel ? ' — ' . $periodLabel : '');
...
// لوگو به‌صورت متن (مثل فاکتور معمولی) تا بدون وابستگی به فایل SVG به‌درستی چاپ شود.
    ...
    <table style="width: 100%; border: none; margin-bottom: 20px;">
        <tr>
            ...
            <td style="width: 50%; vertical-align: top; padding-right: 10px; border: none;">
                <div class="section" style="margin-bottom: 0;">
                    <div class="section-title">اطلاعات طراح</div>
                    <p style="margin: 5px 0;">
                        <span class="info-label">نام:</span> {$designerName}<br>
...
$mpdf->WriteHTML($html);
$filename = 'designer_invoice_' . $invoice['invoice_number'] . '.pdf';
