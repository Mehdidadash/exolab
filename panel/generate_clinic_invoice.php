<?php
// panel/generate_clinic_invoice.php
...
$clinics = db()->query("SELECT id, full_name FROM users WHERE role='clinic' AND active=1 ORDER BY full_name")->fetchAll();
$bankAccounts = getAllBankAccounts();
$message = '';
$generatedInvoiceId = null;

$clinicId = isset($_POST['clinic_id']) ? (int)$_POST['clinic_id'] : 0;
...
if (($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generate'])) || $selectedCasesOnly) {
    ...
    if ($periodType === 'custom') {
        ...
        $message = 'لطفاً کلینیک و بازه زمانی را انتخاب کنید.';
    }

    if (empty($message) && $startDate !== '' && $endDate !== '') {
        $cases = getUninvoicedCasesForClinic($clinicId, $startDate, $endDate);
        ...
        if (empty($cases)) {
            ...
            $clinic = db()->prepare('SELECT full_name FROM users WHERE id = ?');
            $clinic->execute([$clinicId]);
            $clinicName = $clinic->fetchColumn() ?: 'کلینیک';
            ?>
            <!DOCTYPE html>
            <html dir="rtl" lang="fa">
            <head>
                <meta charset="UTF-8">
                <title>پیش‌نمایش فاکتور کلینیک</title>
                ...
                <div class="preview-box">
                    <form id="preview-form" method="post" action="">
                        ...
                        <?php foreach ($groups as $gKey => $group): ?>
                            ...
                            <table>
                                <thead><tr><th style="width:40px;">انتخاب</th><th>#</th><th>بیمار</th><th>خدمت</th><th>تعداد</th><th>جمع</th></tr></thead>
                                <tbody>
                                <?php foreach ($group['cases'] as $c): ?>
                                    <tr>
                                        ...
                                        <td><?= formatAmountToman($c['total_price']) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
                            </table>
                        ...
                        <div style="margin-top:14px; font-weight:bold; font-size:1.1rem;">
                            جمع کل: <?= formatAmountToman($total) ?>
                        </div>
                        <div style="margin-top:16px; display:flex; gap:10px; flex-wrap:wrap;">
                            ...
                            <a href="generate_clinic_invoice.php" class="btn" style="background:#E5E7EB; color:#0F172A;">تغییر بازه/کلینیک</a>
                        </div>
                    </form>
                </div>
            </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm']) && $_POST['confirm'] == 1) {
    ...
    if (!empty($cases)) {
        $invoiceId = createClinicInvoice($clinicId, $cases, date('Y-m-d'), $periodLabel, $bankAccountId);
        $generatedInvoiceId = $invoiceId;
        $message = 'فاکتور کلینیک با موفقیت ایجاد شد.';
    } else {
...
panel_layout_start('صدور فاکتور کلینیک');
?>
        ...
        <div class="form-group">
            ...
            <select id="clinic_id" name="clinic_id" required>
                ...
                <?php foreach ($clinics as $cl): ?>
                    <option value="<?= $cl['id'] ?>" <?= $cl['id'] == $clinicId ? 'selected' : '' ?>>
